...
#define GC9107_VDVSET 0xC4
#define GC9107_FRCTR2 0xC6
...
static const char *TAG = "gc9107_display_driver";
...
{
    writecommand(spi, GC9107_CASET);
    spi_device_acquire_bus(spi->spi_disp.handle, portMAX_DELAY);
    spi_display_write(&spi->spi_disp, 32, (x << 16) | ((x + width) - 1));
    spi_device_release_bus(spi->spi_disp.handle);

    writecommand(spi, GC9107_RASET);
    spi_device_acquire_bus(spi->spi_disp.handle, portMAX_DELAY);
...
static void set_rotation(struct SPI *spi, int rotation)
{
    ...
    switch (rotation) {
        case 0:
            madctl |= GC9107_MADCTL_MX;
            break;
        case 1:
            madctl |= GC9107_MADCTL_MV;
            break;
        case 2:
            madctl |= GC9107_MADCTL_MY;
            break;
        case 3:
            madctl |= GC9107_MADCTL_MV | GC9107_MADCTL_MY | GC9107_MADCTL_MX;
            break;
    }

    writecommand(spi, GC9107_MADCTL);
    writedata(spi, madctl);
}

static void display_init_std(struct SPI *spi)
{
    writecommand(spi, GC9107_SLPOUT);
    delay(120);

    writecommand(spi, GC9107_MADCTL);
    writedata(spi, TFT_MAD_COLOR_ORDER);

    writecommand(spi, GC9107_COLMOD);
    writedata(spi, 0x55);
    delay(10);

    set_rotation(spi, spi->rotation);

    writecommand(spi, GC9107_DISPON);
    delay(120);
}

Context *gc9107_display_create_port(GlobalContext *global, term opts)
{
...
static void display_init(Context *ctx, term opts)
{
    ...
    if (!reset_configured) {
        writecommand(spi, GC9107_SWRESET);
        delay(100);
    }
